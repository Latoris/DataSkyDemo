    ...
    try:
        ...
        for e in data["data"]:
            ...
            if 'range' in detailkeys:
                range = e['range']
        detail_info= Detail_Info(e["mac"],e['rssi'],range, ts,tmc,tc,ds,essid1,essid2,essid3,essid4,essid5,essid6)
        detail_info_list.append(detail_info)
        mslove_data = slove_data()
        mslove_data.save_data(masterInfo,detail_ingo_list)
        end = time.time()
        logging.info(u'end request total time: %d', int(end - start))
    except:
        import sys
